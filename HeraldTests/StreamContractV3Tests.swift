...
private let fixtureFiles = [
    "stream_v3_text_only.json",
    "stream_v3_reasoning.json",
    "stream_v3_multi_tools.json",
    "stream_v3_commentary.json",
    "stream_v3_approval.json",
    "stream_v3_error.json",
    "stream_v3_cancelled.json",
    "stream_v3_goal_continuation.json",
]

// MARK: - Tests

@Suite("Stream Contract v3 golden fixtures")
struct StreamContractV3Tests {
    ...
    @Test("contractVersion is always 3", arguments: fixtureFiles)
    func contractVersionIsThree(filename: String) throws {
        let events = try loadFixture(named: filename)
        for event in events {
            #expect(
                event.contractVersion == 3,
                "Expected contractVersion=3, got \(event.contractVersion) (seq=\(event.seq), type=\(event.type))"
            )
        }
    }
    ...
    @Test("conversationRevision is present and positive", arguments: fixtureFiles)
