...
```bash
# On MBP (herald-dev@build-host.internal):
...
ssh fihadmin@herald-host.internal "cp /home/fihadmin/hermes-ios-work/relay/app/services.py /home/fihadmin/hermes-ios-work/relay/app/main.py /home/fihadmin/Hermes-iOS/relay/app/"
...
```bash
ssh fihadmin@herald-host.internal "systemctl --user status hermes-mobile-connector.service --no-pager | head -8"
```
...
```bash
ssh fihadmin@herald-host.internal "CREDS=\$(cat /home/fihadmin/.hermes/profiles/ignyte/home/.hermes-mobile/state.json | python3 -c 'import json,sys; print(json.load(sys.stdin)[\"connector_credential\"])') && curl -s http://localhost:8010/v1/connector/phone-pairing-codes -X POST -H \"Authorization: Bearer \$CREDS\""
```
