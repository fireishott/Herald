...
// MARK: - Herald 2.1 Brand Surfaces
//
// Shared background primitives for the Herald identity. Everything here is drawn
// procedurally from theme tokens, so one implementation reads correctly in
// Herald (deep cobalt), Herald OLED (true black), and Herald Light — the palette
// supplies the ground, the glow, the texture weight, and the watermark opacity.
//
// Texture rules (BRAND_SYSTEM / BUILDER_THEME_PROMPT):
//   * backgrounds only — never behind dense reading content
//   * 3–8% in-app for the default theme, 0–3% for OLED
    ...
    var body: some View {
        ...
        if resolved > 0 {
            Canvas { context, size in
                var generator = HeraldSeededGenerator(seed: seed)
                for _ in 0 ..< density {
...
/// The signature Herald background: deep ink ground, a restrained blue field
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                ...
                // Blue field. A soft rise rather than a neon glow: the icon's
                ...
                if showsWatermark {
                    ...
                    Image(HeraldTheme.Mark.iconImage)
                        .resizable()
                ...
                if showsTexture, !(isReadingSurface && p.prefersSharpEdges) {
                    HeraldReliefTexture(opacity: p.textureOpacity)
                }
            }
            .ignoresSafeArea()
        }
...
struct HeraldSeededGenerator: RandomNumberGenerator {
